...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        if True:
            ...
            print('patch_tokens.shape',len(patch_tokens),patch_tokens[0].shape)  #patch_tokens.shape 4 torch.Size([52, 1370, 768])  
            #are_equal = torch.equal(patch_tokens[0],patch_tokens[3])
            #are_equal = torch.allclose(patch_tokens[0],patch_tokens[3])
            #print(f"tensor1 和 tensor2 是否相等: {are_equal}") 

...
def compute_similarity(image_features, text_features, t=2):
    # image_features[:, :1, :].shape torch.Size([8, 1, 768])
    # image_features  torch.Size([8, 1370, 768])
    # text_features  torch.Size([2, 768])
    prob_1 = image_features[:, :1, :] @ text_features.t() 
    b, n_t, n_i, c = image_features.shape[0], text_features.shape[0], image_features.shape[1], image_features.shape[2]  # 8,2 ,1370,768
    feats = image_features.reshape(b, n_i, 1, c) * text_features.reshape(1, 1, n_t, c)  # 8,1370, 1, 768   * 1, 1, 2, 768
    print('image_features.reshape(b, n_i, 1, c).shape,text_features.reshape(1, 1, n_t, c).shape,feats.shape',image_features.reshape(b, n_i, 1, c).shape,text_features.reshape(1, 1, n_t, c).shape,feats.shape)
    #  torch.Size([8, 1370, 1, 768]) torch.Size([1, 1, 2, 768]) torch.Size([8, 1370, 2, 768])
